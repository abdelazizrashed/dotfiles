...
class Generator:
        ...
        for prop in props:
            if  prop.data_type.dart_type not in DART_TYPES:
                if "List" not in prop.data_type.dart_type:
                    self.imports.append(Utils.to_snake_case(prop.data_type.dart_type))
                else:
                    class_name = prop.data_type.dart_type.split("<")[1].split(">")[0]
    ...
    def from_json_row(data_class: DataClass) -> str:
        if data_class.data_type.dart_type in DART_TYPES:
            return f"{data_class.name}: json[\"{data_class.name}\"],"
        if "List" in data_class.data_type.dart_type:
            class_name = data_class.data_type.dart_type.split("<")[1].split(">")[0]
