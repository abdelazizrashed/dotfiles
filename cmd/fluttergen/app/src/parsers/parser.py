...
class Parser:
    ...
    def parse_data(self) -> List[DataClass]:
        root = DataClass(data_type=self.data_type, name=self.name, value=self.value)
        data_classes = [root]
        for key, value in self.value.items():
            d_type = self.parse_type(value, key, True)
            d_class = DataClass(data_type=d_type, name=key, value=value)
            default_type = self.type_parser(value)
    ...
    def list_parser(self,value, name: str) -> str:
        if len(value) == 0:
            return "List<dynamic>"
        first = value[0]
        f_type = self.type_parser(first) 
        if len(f_type) != 0:
            return f"List<{f_type}>"
        child_type = Utils.to_pascal_case(self.module_prefix + "_" + name + "_model")
